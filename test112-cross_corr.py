...
peak_idx = int ( np.argmax ( corr ) ) ; peak_val = float ( np.abs ( corr[ peak_idx ] ) ) ; print ( f"corr: {peak_idx=}, {peak_val=}" )
peak_idx = int ( np.argmax ( corr_abs ) ) ; peak_val = float ( corr_abs[ peak_idx ] ) ; print ( f"corr_abs: {peak_idx=}, {peak_val=}" )
...
peak_idx = int ( np.argmax ( norm_corr ) ) ; peak_val = float ( norm_corr[ peak_idx ] ) ; print ( f"norm_corr: {peak_idx=}, {peak_val=}" )
# UWAGA! UWAGA! Podobno powinno być tak:
norm_corr_best = (np.abs(corr) ** 2) / (window_energy * sync_sequence_energy + 1e-12)
peak_idx = int ( np.argmax ( norm_corr_best ) ) ; peak_val = float ( norm_corr_best[ peak_idx ] ) ; print ( f"norm_corr_best: {peak_idx=}, {peak_val=}" )
# lub tak
...
peak_idx = int ( np.argmax ( norm_corr_better ) ) ; peak_val = float ( norm_corr_better[ peak_idx ] ) ; print ( f"norm_corr_better: {peak_idx=}, {peak_val=}" )
