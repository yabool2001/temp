...
# ========================================================
# AUTORSKI MODUŁ 1: ZESPOLONA KOMÓRKA BRAMKOWA
# ========================================================
class PureComplexLSTMCell ( nn.Module ) :
    ...
    def forward ( self , x , hx = None ) :
        ...
        # 🔥 BRAMKI POLARNE: Mózg wirtualnego PLL w sztucznej inteligencji
        # Amplituda bramki zachowana [0, 1] chroni przed rozsadzeniem sieci
        # Faza bramki [-pi, pi] pozwala konstelacji BPSK bezkarnie wirować!
        
        f_mag = torch.sigmoid(f_gate.real)
        f_phase = torch.tanh(f_gate.imag) * torch.pi
        f = torch.polar(f_mag, f_phase)  # Generuje prawdziwą matrycę rotacji
        
        i_mag = torch.sigmoid(i_gate.real)
        i_phase = torch.tanh(i_gate.imag) * torch.pi
        i = torch.polar(i_mag, i_phase)
        
        o_mag = torch.sigmoid(o_gate.real)
        o_phase = torch.tanh(o_gate.imag) * torch.pi
        o = torch.polar(o_mag, o_phase)
        
        c_tilde = torch.complex(torch.tanh(c_gate.real), torch.tanh(c_gate.imag))

        # Prawdziwe ZESPOLONE mnożenie: f * c naturalnie kompensuje CFO!
...
# ========================================================
# AUTORSKI MODUŁ 2: SILNIK CZASOWY (Zastępuje nn.LSTM)
# ========================================================
...
class HardcoreComplexEqualizer ( nn.Module ) :
    ...
    def __init__ ( self ) :
        ...
        # UWAGA: stride = sps oraz padding = 2 a wcześniej było padding = 'same'
        # Zrzucamy maski: z 4096 sampli wejściowych wyjdzie równe 4096 
        # sampli po splotach. Brak utraty rozdzielczości czasu!
        # # kernel_size nieparzysty, by 'same' działało!
        # stride = 1, by nie było decymacji!
        self.conv1 = nn.Conv1d ( in_channels = 1 , out_channels = 16 , kernel_size = 2 * sps + 1 , stride = 1 , padding = 'same' , dtype = torch.complex64 )
