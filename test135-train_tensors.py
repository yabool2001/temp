...
for timestamp_group in timestamp_groups :
	...
	if rx_samples.frames is not None and len ( rx_samples.frames ) > 0 :
		...
		''' W rx_samples znajdź frame, która jest identyczna jak pierwsza znalezion frame w tx_samples (czyli pierwsza ramka w pliku {timestamp_group}_tx_samples_4_pluto.npy)
			...
			for tx_frame in tx_samples.frames :
				for rx_frame in rx_samples.frames :
					...
					if np.array_equal ( rx_frame.header_bits , tx_frame.header_bits ) :
						...
						if dbg : print ( f"\r\nWariant 2. Znaleziono dopasowanie ramki: {timestamp_group=} {tx_frame.frame_start_abs_idx=}, {rx_frame.frame_start_abs_idx=}, {first_active_rx_sample_idx=}" )
						break
	...
	if first_active_rx_sample_idx is not None :
		...
		rx_samples.y_train_np_array [ first_active_rx_sample_idx : first_active_rx_sample_idx + tx_active_symbols.size ] = tx_active_symbols
		if plt : plot.flat_tensor_v0_1_18 ( rx_samples.y_train_np_array , title = f"{script_filename} {timestamp_group} rx y_train_np_array" , marker_idx = first_active_rx_sample_idx )
		...
		if wrt : rx_samples.clip_Xy_samples_wo_mute_for_training ( frames_first_sample_idx = first_active_rx_sample_idx , dir_name = "training" ,  timestamp_group = timestamp_group )
